...
import com.andy.httpserver.TheRequest;
...
public class RoutesShould {
    ...
    void find_route() {
        ...
        Route route = routes.getRoute(new TheRequest("localhost", "/route1", HttpMethod.GET, ""));
    ...
    void not_find_route() {
        Routes routes = new Routes();
        routes.addRoute("/route1", new Route(HttpMethod.GET, "somefink here"));
        Route route = routes.getRoute(new TheRequest("localhost", "/route1", HttpMethod.POST, ""));
        assertEquals("somefink here", route.getBody());
    }
